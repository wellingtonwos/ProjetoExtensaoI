...
\begin{itemize}
    ...
    \item \textbf{Segurança da Informação (\textit{Privacy by Design}):} Todas as medidas técnicas de segurança, como a \textbf{criptografia em repouso (bcrypt)} e a \textbf{criptografia em trânsito (SSL/HTTPS)}, são o mecanismo técnico para cumprir o Artigo 46 da LGPD, garantindo que os dados pessoais estejam protegidos contra acessos não autorizados e incidentes de segurança.
    
    \item \textbf{Transparência e Rastreabilidade (\textit{Accountability}):}
    \begin{itemize}
        \item O sistema mantém \textbf{logs de auditoria} robustos e imutáveis no banco de dados (PostgreSQL/AWS RDS), registrando o acesso e alterações feitas por cada usuário (caixa ou gerente). Isso permite a rastreabilidade completa e o cumprimento de obrigações de prestação de contas.
        \item O \textbf{Consentimento} é obtido de forma clara: o registro de novos usuários administradores implica na aceitação da política de privacidade e uso do sistema, que detalha como os dados de login serão tratados.
    \end{itemize}
...
O presente Trabalho de Conclusão de Curso (TCC) alcançou seu objetivo geral, que era desenvolver o \textsf{CarneUp}, um Sistema de Gestão focado na rastreabilidade e no controle de perecibilidade de estoque para açougues de pequeno e médio porte.
